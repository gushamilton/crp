...
```{r}
common_final <- common_prs_genome %>%
  left_join(final) %>%
  drop_na(crp_prs_scaled_genome) %>%
  mutate(crp_prs_scaled = crp_prs_scaled_genome) %>%
  left_join(principle_components)


common_final %>%
  mutate(decile = ntile(crp_prs_scaled, 4)) %>%
  group_by(decile) %>%
  summarise(crp_m = median(c_reactive_protein, na.rm = T))
...
f <- as.formula(sprintf(paste0(x, "~ crp_prs_scaled + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + age_at_entry + sex ")))
m <- (coxph(formula = f, data = common_final))
m2 <- tidy(m, exponentiate = T, conf.int = T) %>% mutate(model = y) 
return(m2)
}
