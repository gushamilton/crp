...
./regenie \
  ...
  --phenoFile /user/work/fh6520/regenie/regenie-master/was_crp_tested.tsv \
  --covarCol PC{1:10},sex,chip \
  --bsize 1000 \
  --lowmem --bt \
  --lowmem-prefix $SCRATCH_NJT/tmpdir2 \
  --out $SCRATCH_NJT/CRP/testing/testing \
  --gz
...
echo "./regenie \
  ...
  --pred \$SCRATCH_NJT/CRP/testing/testing_pred.list\
  --ref-first \
  --minMAC 100 \
--covarFile /user/work/fh6520/regenie/regenie-master/data.covariates.plink.txt \
  --phenoFile /user/work/fh6520/regenie/regenie-master/was_crp_tested.tsv \
  --covarCol PC{1:10},sex,chip \
  --bsize 400 --bt \
  --out \$SCRATCH_NJT/CRP/testing/$chr \
  --gz" >>  $chr.sh ;
