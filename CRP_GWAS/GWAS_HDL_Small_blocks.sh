...
./regenie \
  ...
  --phenoFile /user/work/fh6520/regenie/regenie-master/hdl_small_measures.tsv \
  --covarCol PC{1:10},sex,chip \
  --bsize 1000 \
  --lowmem \
  --lowmem-prefix $SCRATCH_NJT/tmpdir \
  --out $SCRATCH_NJT/small_hdl/small \
  --gz
...
echo "./regenie \
  ...
  --pred \$SCRATCH_NJT/small_hdl/small_pred.list\
  --ref-first \
  --minMAC 100 \
--covarFile /user/work/fh6520/regenie/regenie-master/data.covariates.plink.txt \
  --phenoFile /user/work/fh6520/regenie/regenie-master/hdl_small_measures.tsv \
